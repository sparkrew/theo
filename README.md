...
2. Set the following configs in the `settings.conf` file.
   
   * `PROJECT_ROOT_DIR` 
   * `PROJECT_SOURCE_CODE_PATH`
   * `PROJECT_JAR_PATH`
   * `PROJECT_PACKAGE_NAME`
   * `THEO_STATIC_ANALYZER_JAR_PATH`
   * `THEO_DYNAMIC_ANALYZER_JAR_PATH`
   * `JFR_SETTINGS_FILE_PATH`
   * `JFR_REPORT_PATH`
   * `THEO_JAVA_AGENT_JAR_PATH`
